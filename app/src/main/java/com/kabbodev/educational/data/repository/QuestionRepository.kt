...
class QuestionRepository {
    ...
    fun getQuestions(selectedChapterIds: ArrayList<String>, selectedQuestionsCount: Int, listener: QuestionCallback) {
        loadQuestions(selectedChapterIds, selectedQuestionsCount, listener)
    }

    private fun getQuestionsById(queSetId: String): Task<QuerySnapshot> =
        questionsCollection.document(queSetId).collection("Ques").get()

    private fun loadQuestions(selectedChapterIds: ArrayList<String>, selectedQuestionsCount: Int, listener: QuestionCallback) {
