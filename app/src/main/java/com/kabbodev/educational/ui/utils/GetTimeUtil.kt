        ...
        fun getDate(timeListener: Listener) {
            GlobalScope.launch(Dispatchers.IO) {
                ...
                if (getTimeUtil.requestTime("time.google.com", 5000)) {
                    ...
                    val fixedTime = "03:00 AM"
                    val fixedDate: Date? = timeFormat.parse(fixedTime)
                    Log.d("Time", "fixedTime $fixedTime")
                    Log.d("Time", "fixedDate $fixedDate")

                    val time: String = timeFormat.format(nowAsPerDeviceTimeZone)
                    val currentTime: Date? = timeFormat.parse(time)
                    Log.d("Time", "currentTime $time")
                    Log.d("Time", "currentTime $currentTime")

                    if (currentTime!!.after(fixedDate)) {
                        timeListener.onTimeReceived(true)
                    } else {
                        timeListener.onTimeReceived(false)
                    }
                }
            }
        }
...
interface Listener {
    fun onTimeReceived(isAfterTime: Boolean?)
